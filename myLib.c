...
mat4 look_at(vec4 eye, vec4 at, vec4 up) {
    vec4 z = normalize_v4(sub_v4(eye,at)); // 
    vec4 x = normalize_v4(cross_product_v4(up,z)); //n
    vec4 y = normalize_v4(cross_product_v4(z, x));
    mat4 view_matrix = {{x.x, y.x,z.x, eye.x},
    {x.y, y.y,z.y, eye.y},
    {x.z, y.z,z.z, eye.z},
    {0, 0,0, 1}};
    return inverse_m4(transpose_m4(view_matrix));
    //n = z prime,
    //q0 is eyex, eyey, eyez 
}
